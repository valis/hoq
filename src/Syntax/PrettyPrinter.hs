...
ppTerm :: Term p String -> Doc
ppTerm t = ppTermCtx (toList t) (fmap text t)

ppTermCtx :: [String] -> Term p Doc -> Doc
ppTermCtx _ (Var d) = d
ppTermCtx _ (Universe _ NoLevel) = text "Type"
ppTermCtx _ (Universe _ l) = text $ "Type" ++ show l
ppTermCtx ctx t@(App e1 e2) = ppTermPrec (prec t) ctx e1 <+> ppTermPrec (prec t + 1) ctx e2
ppTermCtx ctx t@(Pi _ (Type a _) b _) =
...
ppTermCtx _ (ICon _ IRight) = text "right"
ppTermCtx ctx t@(Path _ Implicit _ [e2,e3]) = ppTermPrec (prec t + 1) ctx e2 <+> equals <+> ppTermPrec (prec t + 1) ctx e3
ppTermCtx ctx t@(Path _ _ me es) = text "Path" <+> ppList ctx t (maybe (Var $ text "_") fst me : es)
ppTermCtx ctx t@(PCon _ me) = text "path" <+> maybe empty (ppTermPrec (prec t + 1) ctx) me
ppTermCtx ctx t@(At _ e1 e2) = ppTermPrec (prec t) ctx e1 <+> text "@" <+> ppTermPrec (prec t + 1) ctx e2
ppTermCtx ctx t@(Coe _ es) = text "coe" <+> ppList ctx t es
ppTermCtx ctx t@(Iso _ es) = text "iso" <+> ppList ctx t es
ppTermCtx ctx t@(Squeeze _ es) = text "squeeze" <+> ppList ctx t es
...
ppTermPrec :: Int -> [String] -> Term p Doc -> Doc
...
prec FunCall{}                      = 10
prec (Con _ _ _ _ [])               = 10
prec (DataType _ _ _ [])            = 10
prec (Path _ Explicit Nothing [])   = 10
prec (PCon _ Nothing)               = 10
prec Interval{}                     = 10
prec ICon{}                         = 10
prec (Coe _ [])                     = 10
prec (Iso _ [])                     = 10
...
prec Con{}                          = 9
prec DataType{}                     = 9
prec (Path _ Explicit _ _)          = 9
prec PCon{}                         = 9
prec Coe{}                          = 9
prec Iso{}                          = 9
prec Squeeze{}                      = 9
prec At{}                           = 8
prec (Path _ Implicit _ _)          = 7
prec Pi{}                           = 6
prec Lam{}                          = 5
