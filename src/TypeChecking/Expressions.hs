...
unPosn :: Functor f => f (Scoped p a) -> f (Scoped () a)
unPosn = fmap $ mapScoped $ const ()

termPos :: Context a -> Term Posn a -> Posn
termPos ctx = getAttr $ either (const (0,0)) getPos . toBase ctx

mapScope' :: (p -> p') -> Scope s q (Term p) a -> Scope s q (Term p') a
...
typeCheckCtx ctx term mty = go ctx term [] $ fmap (nfType WHNF) mty
  where
    ...
    go ctx (Var v) ts mty = do
        eres <- case lookupCtx (\s p -> s == getName p) (const ()) v ctx of
            Right (_, ty) -> return $ Left ((0,0), Var v, ty)
            ...
            Left (PIdent pos var, Nothing) -> do
                mt <- lift $ getEntry var $ case mty of
                    ...
                    _                                 -> Nothing
                case mt of
                    [FunctionE (FunCall _ name clauses) ty] -> return $ Left (pos, FunCall () name clauses, fmap (liftBase ctx) $ mapType (const ()) ty)
                    [FunctionE (FunSyn _ name expr) ty]     -> return $ Left (pos, FunSyn  () name expr   , fmap (liftBase ctx) $ mapType (const ()) ty)
                    DataTypeE ty e : _                      -> return $ Left (pos, DataType () var e []   , fmap (liftBase ctx) $ mapType (const ()) ty)
                    [ConstructorE _ (ScopeTerm con) (ScopeTerm ty, lvl)] ->
                        return $ Left (pos, fmap (liftBase ctx) $ mapTerm (const ()) con
                                    , Type (fmap (liftBase ctx) $ mapTerm (const ()) ty) lvl)
                    [ConstructorE _ con (ty, lvl)] -> case mty of
                        Just (Type (DataType _ _ _ params) _) ->
                            ...
                            in  return $ Left (pos, liftTerm con, Type (liftTerm ty) lvl)
    ...
    go ctx (Lam p (Scope1 v t)) [] (Just ty@(Type (Pi p' a b lvl2) _)) = do
        (te, _) <- go (Snoc ctx v a) (unPosn t) [] $ Just $ Type (nf WHNF $ unScope1 $ dropOnePi p' a b lvl2) lvl2
        return (Lam () $ Scope1 v $ unPosn te, ty)
    ...
    go ctx (Lam p (Scope1 v t1)) (t2:ts) mty = do
        ...
        (te1, Type ty1 lvl1) <- go (Snoc ctx v ty2) (unPosn t1) (map (fmap Free) ts) $ fmap (fmap Free) mty
        return (Lam () (Scope1 v $ unPosn te1), Type (Pi () ty2 (Scope v $ ScopeTerm $ unPosn ty1) lvl1) $ max lvl1 lvl2)
...
typeCheckScope :: (Monad m, Eq a) => Context a -> Type () a
    -> Scope String Posn (Term Posn) a -> TCM m (Scope String () (Term ()) a, Level)
...
typeCheckScope ctx a (Scope v b) = do
    (te, ty) <- typeCheckScope (Snoc ctx v a) (fmap Free a) (unPosn b)
...
typeCheckLambda ctx (Lam _ (Scope1 v te)) ty@(Type _ lvl) = do
    (te', Type ty' lvl') <- typeCheckCtx (Snoc ctx v ty) (unPosn te) Nothing
