...
data ScopeState = ScopeState
    ...
    , constructors :: [((Name, ID), (Name, Semantics, [Closed Clause], [SEval], Closed (Type Semantics)))]
    , counter      :: ID
    }
...
addConstructor :: Monad m => Name -> (ID,Name) -> Int -> [Closed Clause] -> [SEval] -> Closed (Type Semantics) -> ScopeT m ()
addConstructor con dt i e es ty = ScopeT $ modify (updScopeConstructor con dt i e es ty)

updScopeConstructor :: Name -> (ID, Name) -> Int -> [Closed Clause] -> [SEval] -> Closed (Type Semantics) -> ScopeState -> ScopeState
...
replaceConstructor :: Monad m => Name -> (ID, Name) -> Int -> [Closed Clause] -> [SEval] -> Closed (Type Semantics) -> ScopeT m ()
...
getConstructor :: Monad m => Name -> Maybe (ID, [Term Semantics a])
    -> ScopeT m [(Name, Term Semantics a, [Closed Clause], [SEval], Type Semantics a)]
