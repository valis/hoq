...
checkTermination :: String -> [PatternC Posn String] -> Closed (Scope String (Term Posn)) -> [EMsg (Term Posn)]
...
checkTerm :: Ctx String (Term Posn) String a -> PatternC Posn String -> Term Posn a -> State Int Ordering
...
checkTerm ctx (PatternVar _) (Var v) = do
  ...
  where
    index :: Ctx String (Term Posn) b a -> a -> Int
    index Nil _ = 0
...
checkTerms :: Ctx String (Term Posn) String a -> [PatternC Posn String] -> [Term Posn a] -> State Int Ordering
...
collectFunCalls :: Ctx String (Term Posn) b a -> String -> [Term Posn a]
    -> Term Posn a -> [(Posn, TermsInCtx String (Term Posn) b)]
...
collectFunCalls ctx name _  (Pi _ (Type e _) s _) = collectFunCalls ctx name [] e ++ go ctx s
  where
    go :: Ctx String (Term Posn) b a -> Scope String (Term Posn) a -> [(Posn, TermsInCtx String (Term Posn) b)]
