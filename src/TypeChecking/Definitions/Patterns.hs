...
typeCheckPattern :: (Monad m, Eq a) => Ctx String Type String a
    -> Type a -> E.Pattern -> TCM m (Bool, Maybe (TermInCtx a), T.Pattern (Closed (Scope String Term)))
typeCheckPattern ctx (Type T.Interval _) (PatLeft _)  = return (False, Just $ TermInCtx Nil $ ICon ILeft , PatternI ILeft)
typeCheckPattern ctx (Type T.Interval _) (PatRight _) = return (False, Just $ TermInCtx Nil $ ICon IRight, PatternI IRight)
typeCheckPattern ctx (Type (DataType _ 0 _) _) (PatEmpty _) = return (True, Nothing, PatternVar "_")
typeCheckPattern ctx (Type ty _) (PatEmpty (PPar (lc,_))) =
    throwError [emsgLC lc "" $ pretty "Expected non-empty type:" <+> prettyOpen ctx ty]
typeCheckPattern ctx _ (PatVar (NoArg _)) = return (False, Nothing, PatternVar "_")
typeCheckPattern ctx ty@(Type (DataType dt _ params) _) (PatVar (Arg (PIdent (lc,var)))) = do
...
typeCheckPattern ctx ty (PatVar (Arg (PIdent (lc,var)))) =
    return (False, Just $ TermInCtx (Snoc Nil var ty) $ T.Var Bound, PatternVar var)
typeCheckPattern ctx (Type (DataType dt _ params) _) (E.Pattern _ (PatName (PIdent (lc,conName)) pats)) = do
...
typeCheckPatterns :: (Monad m, Eq a) => Ctx String Type String a -> Type a -> [E.Pattern]
...
getVariables :: E.Pattern                  -> [PIdent]
getVariables (E.Pattern _ (PatName _ pats)) = pats >>= getVariables
getVariables (PatVar (Arg var))             = [var]
getVariables _                              = []
